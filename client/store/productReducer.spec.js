...
describe('thunk creators', () => {
  ...
  describe('Fetch Products', () => {
    it('eventually dispatches the SET Products action', async () => {
      ...
      expect(actions[1].productList).to.be.deep.equal(fakeProducts)
    })
  })
  ...
  describe('Filters Products', () => {
    ...
    it('has a working action creater', ()=> {
      expect(action).to.deep.equal({type: FILTER_PRODUCTS, products: 'Family Vacation' })
    })
     const newState = reducer(initialState, action)
    it('filters products by category', () => {
      expect(newState.list).to.deep.equal("Family Vacation")
    })
   })
})
